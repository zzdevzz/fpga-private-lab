...
def convert_bmp_to_coe(file_name):
    """Converts a 24-bit BMP to an 8-bit grayscale COE file (1 pixel = 1 address)."""
    ...
    try:
        with open(file_name, "rb") as infile:
            ...
            for _ in range(height):
                ...
                row.reverse()  # BMP rows are stored left-to-right but bottom-up
                grayscale.extend(row)

            grayscale.reverse()  # Flip vertically to correct orientation

        # Write to .coe file with 8-bit words
        ...
        with open(coe_file, "w") as out:
            ...
            for i, val in enumerate(grayscale):
                word = f"{val:02X}"  # 2 hex digits only!
