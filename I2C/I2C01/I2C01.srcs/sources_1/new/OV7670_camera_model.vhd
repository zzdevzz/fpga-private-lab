----------------------------------------------------------------------------------
-- Engineer: ChatGPT
-- Module: OV7670_camera_model
-- Purpose: Simulation model for OV7670 camera
...
end OV7670_camera_model;

architecture Behavioral of OV7670_camera_model is
  ...
  signal vert_cnt   : integer range 0 to V_RES-1 := 0;
...
begin
  if rising_edge(clk) then
    if pclk_int = '1' then
      ...
      -- If in vertical blank, wait a few lines
      if vert_cnt = V_RES and horiz_cnt = 0 and byte_phase = '0' then
        vsync_int <= '1';  -- Start VSYNC pulse
      elsif vert_cnt = V_RES+1 and horiz_cnt = 0 and byte_phase = '0' then
        vsync_int <= '0';  -- End VSYNC pulse, new frame starts
        vert_cnt <= 0;     -- Reset vertical counter to top row
      end if;

      -- HREF for active lines only
      if vert_cnt < V_RES then
        ...
        href_int <= '0';  -- Not sending valid lines
        byte_phase <= '0'; -- Reset byte phase in blanking

        -- Keep incrementing vert_cnt for blank lines
        if horiz_cnt < H_RES-1 then
          horiz_cnt <= horiz_cnt + 1;
        else
          horiz_cnt <= 0;
